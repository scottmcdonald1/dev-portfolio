...
export default function About() {
  ...
  const welcomeMessage = `Hi, I'm Scott McDonald. I'm a full-stack developer looking for work.`
  return (
    <div>
      <Head>
        <title>About</title>
      ...
      <div className="w-screen px-2 pt-20">
        ...
        <div className='font-monda grid items-center py-4'>
          <div 
            className='bg-white/75 max-w-prose mx-auto text-justify grid gap-3 border border-ombreNaturelle31 rounded shadow-sharp px-6 py-4'
          >
            <h1 className='font-bowlbyOneSC text-3xl sm:text-4xl drop-shadow-lg'>About Me</h1>
...
function TextSection({subtitle, section}) {
  ...
  const paragraphs = section.map((paragraph, i) => {
    return <p key={i} className='text-ombreNaturelle31 px-3'>{paragraph}</p>
  })
...
const section1 = [
  `In 2020 during the first lock-down, I began teaching myself the skills needed to enter the web development field. At the time, I was waiting tables at a restaurant that was going through a series of closures in the early stages of the COVID-19 pandemic. So, I enrolled in Harvard's CS50x taught by David Malan and have been coding ever since.`
  ,
  ...
  ,
  `Outside of a formal class environment, I like to live by a learn-by-doing philosophy and have been working on small apps when time allows. My most recent side project is a Spotify API Assistant, a simple utility that allows users to navigate the Spotify API with a friendly search UI. The idea for the app was the product of a conversation I had with my partner, who works for a small record label in the Americana/Country/Folk world, and was a solution to a problem she had concerning ISRC codes and their accessibility.`
  ,
  `I am eager to apply the skills that I have developed in a professional environment. I am proficient in JavaScript and TypeScript and am most familiar with React, but I also have experience working in Angular. I also have ample experience with the back-end and have built apps in Java with Spring Boot.`
  ,
]

const section2 = [
  `I have a BA in Theatre from the University of Missouri and spent a short stint in New York immediately following graduation. Though I don't do too much theatre anymore, I have found other ways to sate my creativity and enjoy drawing and painting. Mostly, my artwork consists of little doodles in marker and pen aimed to make my partner laugh, but occasionally break out the acrylics for portraiture.`
  ,
